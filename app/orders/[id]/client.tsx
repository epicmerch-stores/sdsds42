...
export default function OrderDetailPage() {
  const { store, user } = useStore(); const params = useParams(); const router = useRouter(); const sp = useSearchParams();
  ...
  useEffect(() => {
    ...
    store.orders.get(params.id as string).then(setOrder).catch(() => setOrder(null)).finally(() => setLoading(false));
  }, [store, user, params.id]);
