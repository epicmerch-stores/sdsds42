...
export default function ProductDetailPage() {
  const { store, user, refreshCart } = useStore(); const params = useParams(); const router = useRouter();
  ...
  useEffect(() => {
    ...
    store.products.get(params.id as string).then((p: any) => { setProduct(p); if (p.variants?.length>0) setSelectedVariant(p.variants[0]); }).catch(() => setProduct(null)).finally(() => setLoading(false));
  }, [store, params.id]);
